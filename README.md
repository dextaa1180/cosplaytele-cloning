...
1. **Detail Pages are Preview-Only with Admin-Managed Content**: Detail pages (`/[slug]`) show Cosplaytele-style layout with dark hero section, metadata panel, preview photo/video grid, and download buttons. Preview media and download links are **admin-managed** (not hardcoded). Posts without preview media show thumbnail placeholder with message: "Preview media will be managed from the admin dashboard." Posts without download links show message: "Download links will be added from the admin dashboard." This is a design clone with admin-ready frontend, not a content hosting platform. See `docs/research/tunacosplay/ADMIN_DASHBOARD_PLAN.md` for future admin dashboard plans.
